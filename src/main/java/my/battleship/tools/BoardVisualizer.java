...
public class BoardVisualizer {
    ...
    public static void main(String[] args) throws ParserConfigurationException {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("--directory")) {
                visualizer.boardDir = Paths.get(args[++i]);
            } else if ("--columns".equals(args[i])) {
                visualizer.columns = Integer.parseInt(args[++i]);
            } else {
        ...
        if (visualizer.boardDir != null) {
            visualizer.printBoards();
        } else {
    ...
    public static void printHelp() {
        System.out.println("Usage: BoardVisualizer --directory <dir> ");
        System.out.println("         [--columns <num>] [--help]");
    }
    ...
    private void printBoards() throws ParserConfigurationException  {
        if (boardDir != null) {
            System.out.println("Loading boards...");

            // Board will be 30x30 when printed so if a file name is longer, adjust columns to accomodate
            int maxLineSize = 31;

            try {
                Stream<Path> directoryStream = Files.walk(boardDir, Integer.MAX_VALUE);
                List<Path> filePaths = directoryStream.filter(Files::isRegularFile).sorted().collect(Collectors.toList());
                ...
                for (Path f : filePaths) {
                    ...
                    Board b = new Board(filename.toString(), numRows, numCols);
                    if(filename.length() > maxLineSize) {
                        maxLineSize = filename.length();
                    }
                    boards.add(b);
            ...
            for (int i = 0; i < columns ; ++i) {
                printfArgs += "%-" + String.valueOf(maxLineSize) + "s ";
            }
            printfArgs += "\n";


            for(int boardIdx = 0; boardIdx < boards.size(); boardIdx += columns) {
                for(int line = -1; line < 15; ++line){
                    String[] values = getLineFromNBoards(columns, line, boardIdx);
                    System.out.printf(printfArgs, (Object[]) values);
                }
                System.out.println();
            }
        }
    ...
    private String[] getLineFromNBoards(int numBoards, int lineNum, int offset) {
        String[] result = new String[numBoards];

        if(lineNum < 0) {
            for (int i = 0; i < numBoards; ++i) {
                if (i + offset < boards.size()) {
                    result[i] = "";
                    result[i] = boards.get(i + offset).name;
                } else {
                    result[i] = "";
                }
            }
            ...
            for (int i = 0; i < numBoards; ++i) {
                if (i + offset < boards.size()) {
                    Board b = boards.get(i + offset);
                    StringBuilder sb = new StringBuilder();
                    for (int c = 0; c < b.numCols; ++c) {
                        boolean isShipOnSpace = boards.get(i + offset).spaces[lineNum][c];
                        if (isShipOnSpace) {
                            sb.append('S');
                        } else {
                            sb.append('.');
